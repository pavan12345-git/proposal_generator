...
export function parseMarkdownTable(content) {
  ...
  return content.replace(tableRegex, (match) => {
    ...
    const isDevelopmentCostTable = headerRow.includes('Component') && headerRow.includes('Description') && headerRow.includes('Estimate')
    ...
    } else if (isAdditionalFeaturesTable || isDevelopmentCostTable) {
    ...
    // Numeric-aligned columns
    const numericHeaderNames = ['Estimate', 'Amount', 'Estimated Cost']
    const numericIndexes = headerRow
      .map((h, idx) => ({ h, idx }))
      .filter(x => numericHeaderNames.includes(x.h))
      .map(x => x.idx)
    ...
    dataRows.forEach(row => {
      ...
      if (cells.length > 0) {
        ...
        cells.forEach((cell, i) => {
          const numericClass = numericIndexes.includes(i) ? ' class="numeric"' : ''
          htmlTable += `<td${numericClass}>${cell}</td>\n`
        })
        htmlTable += '</tr>\n'
      }
    })
