...
- Use larger filled circles (●) for main categories and smaller hollow circles (◦) for sub-items
...
export async function generateNextSteps(requirements) {
  const prompt = `You are a professional proposal writer. Generate Next Steps as a simple bullet point list with exactly 3-4 actionable items using filled circles (●):
...
**Requirements:**
- Use filled circle bullets (●) followed by space
...
|-------------------|--------|-------|
| [Component 1] | [Cost with currency] | [Brief description] |
| [Component 2] | [Cost with currency] | [Brief description] |

**Note:** Retainer services include ongoing support, maintenance, and optimization within the agreed scope. Additional work beyond the retainer scope will be billed separately at standard hourly rates.
...
- Generate EXACTLY 1-2 retainer fee items (e.g., Monthly Retainer Fee, Technical Support)
- Keep descriptions EXTREMELY SHORT and concise (e.g., "Technical support, troubleshooting, maintenance issues")
- Use appropriate currency symbol based on country and budget range
- Cost estimates should be REALISTIC for standard website maintenance ($50-300/month range)
- Focus on basic website maintenance services, not enterprise-level support
- Notes should be VERY brief and direct (maximum 5-6 words like "Technical support, troubleshooting, maintenance issues")
- Generate costs appropriate for small to medium business website maintenance
- Include the footer note about scope and limitations
- If company, project, or client fields are blank, do not invent names; use generic references like "the client" or "the company".
